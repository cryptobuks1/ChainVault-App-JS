...
async function swapExactFor(web3, tokenA, tokenB, fromSwap, toSwap, deadline) {
  ...
  **/
  ...
  const routerAddress = await (consumable.contracts["SUSHI_ROUTER"]["address"]);
  tx = { from: publicKey, to: routerAddress };
  sushiRouterContract = await (new web3.eth.Contract(consumable.routerContract.abi, routerAddress));
  ...
  **/
  ...
  if (tokenA == "ETH") {
    ...
    return (await sushiRouterContract.methods.swapTokensForExactTokens(toSwap, fromSwap, path,
                                               consumable.publicKey, deadline).send(tx));
  }
}
